...
int json_flatten(struct json_parse_state *p, char *body, int body_len)
{
  ...
  {
    if (p->line_len > (MAX_LINE_LEN - 3))
      p->line_len = (MAX_LINE_LEN - 3);
...
int json_body(int sock, long long timeout_time)
{
  ...
  while (1)
  {
    ...
    if (r > 0)
    {
      if (json_flatten(&parse_state, line, r))
        break;
    }
    else
      usleep(1000);
...
#define LINE_BYTES 65536
  ...
  while (r > -1)
  {
    ...
    if (r > 0)
    {
      ...
      if (last_read_time)
        *last_read_time = gettime_ms();
      fwrite(line, r, 1, outfile);
      rxlen += r;
      if (content_length > -1)
      {
        if (rxlen >= content_length)
          break;
      }
    }
    else
                     ...
                     int timeout_ms)
{
  ...
  int content_length =
      // manifest part
      2 + boundary_len + 2 + strlen(manifest_header) + manifest_length + 2

      // payload part
      + 2 + boundary_len + 2 + strlen(body_header) + body_length + 2 + 2 + boundary_len + 2
                     ...
                     int timeout_ms)
{
  ...
  // Calculate content length
  int content_length = 0 + 2 + boundary_len + 2 + strlen(message_header) + message_length + 2 + 2 + boundary_len + 2 + 2; // not sure where we have missed this last 2, but it is needed to reconcile
