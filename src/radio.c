...
int radio_send_message(int serialfd, unsigned char *buffer, int length)
{
  ...
  {
  case RADIO_RFD900:
    radio_send_message_rfd900(serialfd, out, offset);
    break;
  case RADIO_BARRETT_HF:
    radio_send_message_barretthf(serialfd, out, offset);
    break;
  case RADIO_CODAN_HF:
    radio_send_message_codanhf(serialfd, out, offset);
    break;
  case RADIO_RF95:
    radio_send_message_rf95(serialfd, out, offset);
    break;
  }
...
int radio_receive_bytes(unsigned char *bytes, int count, int monitor_mode)
{
  ...
  if (radio_get_type() == RADIO_RFD900)
    uhf_receive_bytes(bytes, count);
  else if (radio_get_type() == RADIO_CODAN_HF)
    hf_codan_receive_bytes(bytes, count);
  else if (radio_get_type() == RADIO_BARRETT_HF)
    hf_barrett_receive_bytes(bytes, count);
  else if (radio_get_type() == RADIO_RF95)
    rf_receive_bytes(bytes, count);
...
{
  if (debug_radio)
    dump_bytes("packet before decode_rs", packet_data, packet_bytes);
  ...
  {
    if (0)
      printf("CHECKPOINT: %s:%d %s() error counts = %d for packet of %d bytes.\n",
