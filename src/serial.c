...
int serial_resetup_port(int fd) {
  fprintf(stderr, "Serial port reset!\n");
  return serial_setup_port_with_speed(fd, serial_speed);
}
...
int serial_setup_port(int fd)
{
  ...
  */
  ...
  if (sscanf((char *)buf,"VER\r\nCICS: V%d.%d",&verhi,&verlo)==2) {
    ...
    fprintf(stderr, "Autodetecting rf95modem...\n");
    write_all(fd, "AT+HELP\n", 8);
    sleep(1); // give the radio the chance to respond
    count = read_nonblock(fd,buf,8192);  // read reply
    write_all(fd, "AT+INFO\n", 8);
    sleep(1); // give the radio the chance to respond    
    count = read_nonblock(fd,buf,8192);  // read reply
    dump_bytes("modem response",buf,count);
    if (strstr(buf, "status info:") == buf) {
      int maver, miver;      
      char *version;
      char *token = NULL;
      char *str = strdup(buf);
      while ((token = strsep(&str, "\n"))) {
        if(strstr(token, "firmware:") == token) {
          version = token;
          sscanf(version, "firmware: %d.%d", &maver, &miver);
        }
      }
      fprintf(stderr,"RF95modem running firmware version %d.%d.\n", maver, miver);
